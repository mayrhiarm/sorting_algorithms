...
/**
 * get_card_positions - return the position based on card you put in
...
int get_card_positions(deck_node_t *node)
{
	...
	{
		if (_str_cmp((*node).card->value, "Ace") == 1)
			value = 1;
		else if (_str_cmp((*node).card->value, "10") == 1)
			value = 10;
		else if (_str_cmp((*node).card->value, "Jack") == 1)
			value = 11;
		else if (_str_cmp((*node).card->value, "Queen") == 1)
			value = 12;
		else if (_str_cmp((*node).card->value, "King") == 1)
			value = 13;
	}
...
/**
 * insertion_sorts_deck - function that sorts a doubly linked deck
...
void insertion_sorts_deck(deck_node_t **deck)
{
	...
	while (node)
	{
		...
		if (node->prev)
		{
			value_prev = get_card_positions((node->prev));
			value_current = get_card_positions(node);
		}
